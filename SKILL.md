...
description: "AI短片全流程自动制作管线 (V8.4)。L1/L2双参考角色一致性 + 多剧集故事容量 + Seedance 2.0 omni_reference。OpenClaw 是唯一编排引擎，创意生成通过 ACP 调用 hermes-agent 的 16 个活跃 movie-expert + 垂直专家。gold-team 只做 GPU 调度。20步管线，审核门不可跳过。"
---

# kais-movie-agent V8.4 — OpenClaw 编排 + hermes-agent 专家驱动架构
...
图合成   → dreamina image2image --prompt "..." --images L1_face.png L2_costume.png --sample-strength 0.4 --model-version 5.0 --ratio 3:4 --resolution 2k  # 🆕 双参考系统
视频omni → dreamina multimodal2video --prompt "@Image1 锚定身份..." --images L1_*.png scene.png  # 🆕 Seedance 2.0 omni_reference
...
Step 7:  生成主角·L1身份锚点+L2造型卡片 (kais-jimeng-cli) → checkpoint
         🎙️ hermes-agent experts: character_designer (角色设定) + visual_executor (drawer sub-step, 出图 prompt)
        └─ 7A: L1 身份锚点 — 文生图20选3面部特写（黄金标准检测）
        └─ 7B: L1 审核 (>=7) → 通过，注册到 CharacterAssetManager（永不更换）
        └─ 7C: L2 造型卡片 — compositions API（L1参考, sample_strength=0.3）每套服装正+侧
...
Step 17: 云端终版视频（Seedance 2.0 omni_reference） → 🔒 REVIEW GATE
         └─ 📐 V8.3: omni_reference 模式 — L1锚点70%权重 + 场景首帧30%权重
         └─ prompt 使用 @Image N 绑定，零面部描述
...
# 2. visual_executor (drawer sub-step)：把角色设定翻译成 kais-jimeng-cli 可用的 prompt
...
- `lib/jimeng-client.js` — `compositions()` 图生图 + `omniReferenceVideo()` 视频生成
...
const images = await jimengClient.compositions(prompt, {
  images: refPack.images,     // L1在前，L2在后
  sample_strength: 0.4,       // 推荐起始值
});

// 视频生成（Seedance 2.0 omni_reference）
const omniPack = await assetManager.getOmniReferencePack(charId, { costumeId, sceneFrame });
const videoUrl = await jimengClient.omniReferenceVideo(prompt, omniPack);
```
