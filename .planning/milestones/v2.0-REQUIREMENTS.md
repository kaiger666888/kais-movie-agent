...
- [x] **ARCH-02**: `HermesClient.VALID_PHASES` 与新 `PHASES` 数组同步,或改为服务端开放枚举
- [x] **ARCH-03**: 验证 `V2_MIGRATION_MAP` 旧→新 ID 映射正确性,清理 stale 引用 ✓ 2026-06-23 (10-02)
- [x] **ARCH-04**: `character-generation` handler 实际调用 `CharacterAssetManager` 生成 L1 身份锚点(20选3) + L2 造型卡片(compositions API)

### QUAL — 质量保障实化(P0 致命)

- [x] **QUAL-01**: `_getDINOv2Score` 接入真实视觉模型(智谱 GLM-4V 或 gold-team DINOv2),删除 `return 0.85` 假数据
- [x] **QUAL-02**: 删除 `quality-gate.js` 默认 80% 兜底(`Math.round(meta.max * 0.8)`),LLM 失败立即标记评分异常
- [x] **QUAL-03**: `consistency-guard` 阶段实际调用 `auditContinuity()` 并阻断不合格 shot(不再 silent pass)
- [x] **QUAL-04**: `auditImageVsL1` 在场景图/分镜首帧生成后即时触发,阈值 < 0.7 重试

### PERF — 镜头级并行与成本核算(P1)

- [x] **PERF-01**: 实现 `parallel_shots: 4` 真正的 `Promise.all` 镜头级并行调度
- [x] **PERF-02**: GPU 任务 `waitForTask` 阻塞 pipeline(不再"提交即 completed"),轮询间隔 5s,超时 10min
- [x] **PERF-03**: 单集 GPU-分钟成本核算报表(`evaluation-collector` 聚合,产出 `cost-report.json`)
- [x] **PERF-04**: 镜头级失败重试预算(`max_retries: 1` 自适应至 3)

### SAFE — 工程安全(P1)

- [x] **SAFE-01**: `CompositionEngine.compose()` 改用 `execFile` + args 数组,禁止 shell 字符串拼接
- [x] **SAFE-02**: FFmpeg 输入路径与 filter_complex 字符 sanitize(拒绝含 `"`, `` ` ``, `$`, `;` 的路径)
- [x] **SAFE-03**: 删除 CompositionEngine fallback 中的二次字符串拼接降级链
