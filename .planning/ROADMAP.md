...
### Phase 18: V6 stub handler 真实化(12 个) + QUAL-02 测试补充

**Goal:** 实化 v2.0 audit W-1 标记的 12 个 V6 stub handler 真实业务逻辑(主要在创意立项上半 + 部分 production handler),让 E2E 在 degraded 模式下也能产出有意义的内容(不是空数组);同时补充 QUAL-02 单元测试覆盖(W-4)
**Depends on**: Phase 17
**Requirements**: ARCH-01 (closure), QUAL-02 (test coverage)
**Success Criteria** (what must be TRUE):
  1. `pain-discovery` handler 调用 `generateTopics`/`audienceMatch` (hooks 已存在),产出非空 `pain-report.json`(至少 3 个痛点候选)
  2. `topic-selection` / `outline-selection` / `script-selection` / `character-selection` / `scene-selection` handler 实现候选选择逻辑(读 candidates → 按 review 或 LLM 评分选优 → 写 selected-*.json)
  3. `outline-generation` / `script-generation` 调用 LLM(callLLM)产出多候选(degraded 模式下 fallback 到模板生成,不抛 fatal)
  4. `script-lock` handler 透传 selected-script 到 script-locked.json,附审核元数据
  5. `final-audio` handler 实化 BGM/SFX 路径收集(读 bgm-skeleton + temp-dialogue,写 audio-stems.json)
  6. `delivery` handler 实化(已在 Phase 16 部分完成,补充 final.mp4 路径声明 + quality-report 完整字段)
  7. QUAL-02 单元测试覆盖:`QUALITY_GATE_ALL_DIMENSIONS_FAILED` throw path + null-dimension skip 逻辑(至少 4 个新测试用例)
  8. 现有 151 个测试不回归,E2E 测试在 degraded 模式产出非空 content pain-report / outline / script / selected-* 文件
**Plans:** TBD
