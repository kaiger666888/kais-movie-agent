/**
 * @deprecated Phase 13 (2026-06-17) — V8 pipeline orchestrator deprecated.
 * Default is now KAI_PIPELINE_MODE=v2 (see lib/v2_pipeline.js for V2Pipeline).
 * This file is preserved for backward compatibility (set KAI_PIPELINE_MODE=v8 explicitly to use).
...
export class Pipeline {
  constructor(config = {}) {
    ...
    this.canvasClient = config.canvasClient || null;
    this.blueprint = null;
  ...
  async _runRemoteReview(phase, phaseConfig = {}) {
    ...
    try {
      ...
      // 如果配置了 canvasClient 但未显式提供 onCanvasPush，自动创建
      if (!this.onCanvasPush && this.canvasClient) {
        try {
          const { createOnCanvasPush } = await import('./canvas-push.js');
          this.onCanvasPush = createOnCanvasPush(this.canvasClient, {
            branchId: 'main',
            outputBaseUrl: this.config.canvas?.outputBaseUrl || 'http://192.168.71.166:8000/oss',
            localPrefix: this.config.canvas?.localPrefix || '/mnt/agents/output',
            workdir: this.workdir,
            pipelineId: this.traceId,
            episode: this.episode,
          });
        } catch (err) {
          console.warn(`[pipeline] createOnCanvasPush 失败 (降级): ${err.message}`);
          this.onCanvasPush = null;
        }
      }
  ...
  async runPhase(phaseId, phaseConfig = {}) {
    ...
    // Canvas: 推送阶段开始状态（progress placeholder）
    if (this.canvasClient) {
      try {
        const { createOnCanvasProgress } = await import('./canvas-push.js');
        const pushProgress = createOnCanvasProgress(this.canvasClient, {
          branchId: 'main',
          phaseId,
          phaseName: phase.name,
          phaseIndex: phase.stageOrder,
          episode: this.episode,
        });
        await pushProgress('running');
        // 缓存到 phaseConfig 供 success/error 复用
        if (!this._canvasProgressFns) this._canvasProgressFns = new Map();
        this._canvasProgressFns.set(phaseId, pushProgress);
      } catch (err) {
        console.warn(`[pipeline] canvas progress running 失败 (降级): ${err.message}`);
      }
    }
    ...
    try {
      ...
      // Canvas: 推送阶段完成状态
      if (this._canvasProgressFns?.has(phaseId)) {
        try { await this._canvasProgressFns.get(phaseId)('success'); }
        catch (err) { console.warn(`[pipeline] canvas progress success 失败 (降级): ${err.message}`); }
        finally { this._canvasProgressFns.delete(phaseId); }
      }
      ...
      // Canvas: 推送阶段失败状态
      if (this._canvasProgressFns?.has(phaseId)) {
        try { await this._canvasProgressFns.get(phaseId)('error'); }
        catch (err) { console.warn(`[pipeline] canvas progress error 失败 (降级): ${err.message}`); }
        finally { this._canvasProgressFns.delete(phaseId); }
      }
