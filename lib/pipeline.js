...
export class Pipeline {
  ...
  async _executeDefaultPhase(phase, phaseConfig) {
    ...
    if (phase.id === 'requirement') {
      ...
      // Phase 1 后：受众匹配预测（kais-audience）
      try {
        const { audienceMatch } = await import('./audience-adapter.js');
        const matchResult = await audienceMatch({
          content: req,
          platform: this.config.platform || 'douyin',
        });
        this.audienceMatch = matchResult;
        await writeFile(
          join(this.workdir, 'audience-match.json'),
          JSON.stringify(matchResult, null, 2),
        );
        console.log(`[pipeline] ✅ 受众匹配完成（kais-audience）`);
      } catch (e) {
        console.warn(`[pipeline] 受众匹配跳过: ${e.message}`);
      }

      // Phase 1 后：选题发散（kais-topic-selector + 多约束聚合）
      try {
        const { generateTopics } = await import('./topic-selector-adapter.js');
        ...
        this.candidateTopics = topics;
        const topicPath = join(this.workdir, 'candidate-topics.json');
        await writeFile(topicPath, JSON.stringify(topics, null, 2));
        console.log(`[pipeline] ✅ 选题发散完成: ${topics.length} 个候选选题`);
      } catch (e) {
    ...
    // Phase 3: 角色设计 — DNA锚定（强制）：生成身份验证片段 + 注册DNA卡
    if (phase.id === 'character') {
      const characters = phaseConfig.data?.characters;
      if (!characters || characters.length === 0) {
        throw new Error('[pipeline] Phase 3 未产出角色数据，无法生成DNA卡。检查 character-designer 是否正确返回 characters 数组。');
      }

      if (!this.jimengClient) {
        throw new Error('[pipeline] jimengClient 未初始化，无法生成DNA卡。');
      }

      for (const char of characters) {
        const refImages = char.refImages || char.imageUrls || (char.imageUrl ? [char.imageUrl] : []);
        if (!refImages.length) {
          throw new Error(`[pipeline] 角色 ${char.name} 无参考图（refImages/imageUrls/imageUrl 均为空），无法生成DNA卡。`);
        }
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            ...
            if (attempt === 3) {
              throw new Error(`[pipeline] 角色 ${char.name} DNA卡生成失败（3次重试），管线终止。原因: ${e.message}`);
            }
            await new Promise(r => setTimeout(r, 3000)); // 重试间隔3秒
          }
        }
      }

      // 持久化DNA卡到工作目录
      const dnaPath = join(this.workdir, 'character-dna.json');
      await writeFile(dnaPath, JSON.stringify(Object.fromEntries(this.characterDNA), null, 2));
      console.log(`[pipeline] ✅ ${this.characterDNA.size} 个角色DNA卡已保存 → ${dnaPath}`);
    }

    // Phase 4 后：剧本毒点检测 + 情绪曲线验证（kais-audience）
    if (phase.id === 'scenario' && phaseConfig.data) {
      try {
        const { deepAudienceAnalysis } = await import('./audience-adapter.js');
        const scriptContent = typeof phaseConfig.data === 'string'
          ? phaseConfig.data
          : phaseConfig.data.script || JSON.stringify(phaseConfig.data);
        const analysis = await deepAudienceAnalysis({
          script: scriptContent,
          platform: this.config.platform || 'douyin',
        });
        this.audienceAnalysis = analysis;
        await writeFile(
          join(this.workdir, 'audience-analysis.json'),
          JSON.stringify(analysis, null, 2),
        );
        console.log(`[pipeline] ✅ 剧本受众测评完成（kais-audience）`);
      } catch (e) {
        console.warn(`[pipeline] 剧本受众测评跳过: ${e.message}`);
      }
    }

    // Phase 5: 场景图生成 — 场景DNA锚定（可选）：生成锚点帧 + 注册DNA卡
    if (phase.id === 'scene') {
      const scenes = phaseConfig.data?.scenes;
      if (!scenes || scenes.length === 0) {
        console.log('[pipeline] Phase 5 无场景数据，跳过场景DNA卡生成（可选）。');
      } else if (!this.jimengClient) {
        console.warn('[pipeline] jimengClient 未初始化，跳过场景DNA卡生成（可选）。');
      } else {
        const { generateSceneAnchor } = await import('../skills/kais-scene-designer/lib/designer.js');

        for (const scene of scenes) {
          let registered = false;
          for (let attempt = 1; attempt <= 3; attempt++) {
            try {
              const anchor = await generateSceneAnchor(scene, this.jimengClient);
              if (!anchor.imageUrl) throw new Error('锚点帧生成返回空URL');

              this.sceneDNA.set(scene.name || scene.id, {
                seed: anchor.seed,
              ...
              const anchorPath = join(this.workdir, `scene-${scene.name || scene.id}-anchor.png`);
              await this.jimengClient.download(anchor.imageUrl, anchorPath);
              console.log(`[pipeline] ✅ 场景 ${(scene.name || scene.id)} 锚点帧已生成${anchor.seed ? ` (seed: ${anchor.seed})` : ''}`);
              registered = true;
              break;
            } catch (e) {
              console.warn(`[pipeline] 场景 ${(scene.name || scene.id)} 锚点帧第${attempt}次失败: ${e.message}`);
              if (attempt === 3) throw new Error(`[pipeline] 场景 ${(scene.name || scene.id)} DNA卡生成失败（3次重试）。原因: ${e.message}`);
              await new Promise(r => setTimeout(r, 3000));
            }
          }
        }

        const sceneDnaPath = join(this.workdir, 'scene-dna.json');
        await writeFile(sceneDnaPath, JSON.stringify(Object.fromEntries(this.sceneDNA), null, 2));
        console.log(`[pipeline] ✅ ${this.sceneDNA.size} 个场景DNA卡已保存 → ${sceneDnaPath}`);
      }
    }

    // Phase 7: 视频生成前 — 强制传递DNA卡给camera
    if (phase.id === 'camera' || phase.id === 'video') {
      if (this.characterDNA.size === 0) {
        // 尝试从文件恢复（管线中断后重启场景）
        const dnaPath = join(this.workdir, 'character-dna.json');
        try {
          const raw = await readFile(dnaPath, 'utf-8');
          const saved = JSON.parse(raw);
          for (const [name, dna] of Object.entries(saved)) {
            this.characterDNA.set(name, dna);
          }
          console.log(`[pipeline] 从 ${dnaPath} 恢复了 ${this.characterDNA.size} 个DNA卡`);
        } catch {
          throw new Error(`[pipeline] Phase 7 需要DNA卡但无可用数据。确保 Phase 3 已成功执行并生成了 character-dna.json。`);
        }
      }
      ...
      console.log(`[pipeline] ✅ ${this.characterDNA.size} 个DNA卡已注入camera`);

      // 场景DNA（可选）
      if (this.sceneDNA.size === 0) {
        const sceneDnaPath = join(this.workdir, 'scene-dna.json');
        try {
          const raw = await readFile(sceneDnaPath, 'utf-8');
          const saved = JSON.parse(raw);
          for (const [name, dna] of Object.entries(saved)) {
            this.sceneDNA.set(name, dna);
          }
          console.log(`[pipeline] 从 ${sceneDnaPath} 恢复了 ${this.sceneDNA.size} 个场景DNA卡`);
        } catch { /* 场景DNA可选，不阻断 */ }
      }

      if (this.sceneDNA.size > 0) {
        const { registerSceneDNA } = await import('../skills/kais-camera/lib/camera.js');
        for (const [name, dna] of this.sceneDNA) {
          registerSceneDNA(name, dna);
        }
        console.log(`[pipeline] ✅ ${this.sceneDNA.size} 个场景DNA卡已注入camera`);
      }
    }
