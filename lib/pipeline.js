...
const PHASES = [
  ...
  { id: 'outline-generation', name: '大纲生成', stage: 'outline', stageOrder: 2,
    ...
    notes: '多集时为每集生成独立大纲，标注集间关联和预估时长',
    review: false },
  ...
  // Phase 26 PIPE-DATA-02: spatio-temporal-script moved BEFORE scene-generation so
  // scene-generation's bus.read('spatio-temporal-script') (lib/phases/index.js:2564)
  // finds the sts asset already written (lib/phases/index.js:885). Old order was
  // scene-generation(8) → scene-selection(9) → spatio-temporal-script(10) which caused
  // bus.read to return null and scene-generation to fall back to a single default scene.
  { id: 'spatio-temporal-script', name: '时空剧本', stage: 'spatio-temporal', stageOrder: 8,
    outputFiles: ['sts-script.json'],
    review: { title: '时空剧本审核', selectMode: 'single', enableScoring: true, enableFeedback: true } },
  { id: 'scene-generation', name: '场景生成(6图一体)', stage: 'scene-gen', stageOrder: 9,
    outputFiles: ['scene-candidates.json', 'assets/scenes/'],
    review: false },
  { id: 'scene-selection', name: '场景选择→geometry-bed', stage: 'scene-select', stageOrder: 10,
...
// V2/V4.1 → V6 phase ID migration map (validated against PHASES at module load)
...
// 模块加载时立即执行,若 V2_MIGRATION_MAP 引用了 PHASES 中不存在的 id 立即 fail-fast
...
const REQUIREMENT_SCHEMA = {
  ...
  // ─── 多剧集支持 (2026-06-18) ───────────────────────────
  // episode_count 和 duration_sec_per_episode 共同决定总故事量
  // story_synopsis 必须能支撑 episode_count × duration_sec_per_episode 的总时长
  episode_count: 1,                // 剧集数（默认1=单集短片）
  duration_sec_per_episode: 60,    // 每集时长（秒），默认60秒
  // ─── 单集兼容 ──────────────────────────────────────────
  duration_sec: 60,               // 向后兼容：单集总时长（= duration_sec_per_episode × episode_count）
  // ───────────────────────────────────────────────────────
  style_preference: '',
  production_mode: '',
...
export function createRequirementTemplate(overrides = {}) {
  const episode_count = overrides.episode_count || 1;
  const duration_sec_per_episode = overrides.duration_sec_per_episode || 60;
  return {
    ...REQUIREMENT_SCHEMA, ...overrides,
 // 向后兼容：duration_sec = 总时长
    duration_sec: overrides.duration_sec || (episode_count * duration_sec_per_episode),
    episode_count,
    duration_sec_per_episode,
...
export function validateRequirement(req) {
  ...
  if (req.episode_count && req.episode_count < 1) errors.push('episode_count 至少 1');
  if (req.duration_sec_per_episode && req.duration_sec_per_episode < 5) errors.push('duration_sec_per_episode 至少 5 秒');
...
export class Pipeline {
  constructor(config = {}) {
    ...
    this.onCanvasPush = config.onCanvasPush || null;
    this.blueprint = null;
  ...
  async _runRemoteReview(phase, phaseConfig = {}) {
    ...
    try {
      ...
      // Canvas: 将 candidates 推送到画布
      if (this.onCanvasPush && candidates.length > 0) {
        try {
          await this.onCanvasPush(phase, candidates);
        } catch (canvasErr) {
          console.warn(`[pipeline] Canvas push failed (降级): ${canvasErr.message}`);
        }
      }
  ...
  async runPhase(phaseId, phaseConfig = {}) {
    ...
    try {
      ...
      // Phase 17 E2E-01: normalize result — many handlers return undefined
      // (seed-skeleton, motion-preview, ai-preview). Without this guard,
      // `result.summary` throws TypeError and aborts the whole pipeline.
  ...
  async run(phasesConfig = {}) {
    ...
    // Phase 17 E2E-01: re-load state before final save. runPhase() loads/saves its
    // own copy per call, so the `state` captured at the top of run() is stale —
    // saving it here would wipe all per-phase results written by runPhase.
  ...
  async resume(fromPhaseId = null, phasesConfig = {}) {
    ...
    // Phase 17 E2E-01: re-load state before final save (same fix as run()).
    const finalState = await this._loadState();
    finalState.completedAt = new Date().toISOString();
    await this._saveState(finalState);
    const success = Object.values(results).every(r => !r.error);
