...
import { auditContinuity } from '../continuity-auditor.js';

// ─── Hermes Helpers ────────────────────────────────────────────

function _makeCollector(pipeline) {
  return new EvaluationCollector(pipeline.workdir);
}
...
export const phaseHandlers = {
    ...
    after: async (pipeline, phase, phaseConfig) => {
      ...
      for (const shot of shots) {
        try {
          ...
          videoResults.push({ shotId: shot.id, taskId: result.taskId, state: 'submitted' });
  ...
  'character-generation': {
    after: async (pipeline, phase, phaseConfig) => {
      ...
      // Step 3: Ensure assets/characters/ + write character-candidates.json stub
      // Real L1/L2 layered system deferred to Phase 14
      await mkdir(join(pipeline.workdir, 'assets/characters/'), { recursive: true });
      const stubData = {
        _stub: true,
      ...
      await writeFile(join(pipeline.workdir, 'character-candidates.json'), JSON.stringify(stubData, null, 2));

      _hermesAudit(hermes, 'character-generation', hermesDecisionId, { stubbed: true }, effectiveParams);
    ...
    after: async (pipeline, phase, phaseConfig) => {
      ...
      if (hermes) {
        try {
          ...
          console.warn(`[consistency-guard] hermes decide 降级: ${e.message} (将在 Phase 11 修复)`);
        }
      }

      // Step 3: Call auditContinuity (real scoring deferred to Phase 12, this is stub)
      let auditResult = { _stub: true, overall: 1.0, passed: true, findings: [] };
      try {
        const r = await auditContinuity({
          visuals: [],
          characterAssets: [],
          goldTeamClient: null,
        });
        ...
        console.warn(`[consistency-guard] auditContinuity 降级(Phase 12 将实化): ${e.message}`);
      }
      const stubData = {
        _stub: true,
        _phase: 'consistency-guard',
        _generatedAt: new Date().toISOString(),
        _pendingRealImplementation: 'phase-12',
        ...auditResult,
      };
      await writeFile(join(pipeline.workdir, 'consistency-pass.json'), JSON.stringify(stubData, null, 2));

      _hermesAudit(hermes, 'consistency-guard', hermesDecisionId, { stubbed: true }, effectiveParams);
      ...
      return { summary: stubData, metrics: { stubbed: true, _pendingRealImplementation: 'phase-12' } };
    },
  },
  ...
  'cloud-production': {
    after: async (pipeline, phase, phaseConfig) => {
      ...
      // Step 3: Ensure final-shots/video/ + write video_tasks.json stub
      // Real Seedance omni_reference deferred to Phase 15
      ...
      _hermesAudit(hermes, 'cloud-production', hermesDecisionId, { stubbed: true }, effectiveParams);
  ...
  'delivery': {
    after: async (pipeline, phase, phaseConfig) => {
      ...
      // Step 3: Call assessQuality + CompositionEngine.runQualityCheck, write quality-report.json stub
      let qualityReport = {
        _stub: true,
        overall_score: 0,
        dimensions: {},
      ...
      await writeFile(join(pipeline.workdir, 'quality-report.json'), JSON.stringify(stubData, null, 2));

      _hermesAudit(hermes, 'delivery', hermesDecisionId, { stubbed: true }, effectiveParams);
      ...
      return { summary: stubData, metrics: { stubbed: true, _pendingRealImplementation: 'phase-13' } };
    },
  },
};
