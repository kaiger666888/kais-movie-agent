...
export const phaseHandlers = {
        ...
        try {
          ...
          if (await gtClient.ping(5000)) {
            for (const shot of stsScript.shots || []) {
              const result = await generateArtDirectionViaGoldTeam(pipeline, shot.description, artBible.style_anchor);
              frameResults.push({ shot_id: shot.id, taskId: result.taskId });
            }
          }
    ...
    after: async (pipeline, phase, phaseConfig) => {
      ...
      for (const shot of shots) {
        try {
          ...
          const result = await generateVideoViaGoldTeam(pipeline, { ...shot, description: enhancedPrompt, _preview: true });
  ...
  'final-production': {
    after: async (pipeline, phase, phaseConfig) => {
      ...
      for (const shot of shots) {
        try {
          const enhancedPrompt = await injector.inject(shot.description, { character: shot.character, scene: shot.scene_id, shotId: shot.id });
          const result = await generateVideoViaGoldTeam(pipeline, { ...shot, description: enhancedPrompt });
      ...
      // Signature BGM (YuE 7B) for marked shots
      const bgmStrategy = new BGMStrategy({ assetBus: bus, goldTeamClient: _makeGtClient(pipeline) });
      const stsScript = await bus.read('spatio-temporal-script') || {};
      const artBible = await bus.read('art-bible') || {};
      for (const shot of stsScript.shots || []) {
  ...
  composition: {
    after: async (pipeline, phase, phaseConfig) => {
      ...
      const composer = new CompositionEngine({ workdir: pipeline.workdir, config: pipeline.config });
...
export async function generateArtDirectionViaGoldTeam(pipeline, prompt, style) {
  ...
  const defaults = HERMES_DEFAULTS['art-direction'];
  const { params: hermesParams, decisionId } = await _hermesDecide(hermes, 'art-direction', {
...
export async function generateVideoViaGoldTeam(pipeline, shot) {
  ...
  const { params: hermesParams, decisionId } = await _hermesDecide(hermes, 'camera', {
...
export async function generateBGM(pipeline, prompt, duration = 60) {
  ...
  const { params: hermesParams, decisionId } = await _hermesDecide(hermes, 'post-production', {
...
export async function generateSFX(pipeline, prompt) {
  ...
  const defaults = HERMES_DEFAULTS['post-production'].sfx;
  const { params: hermesParams, decisionId } = await _hermesDecide(hermes, 'post-production', {
    prompt, task: 'sfx',
  });
