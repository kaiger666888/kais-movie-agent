...
// ─── JimengClient Deprecation (Phase 27-02, D-PIPE-RENDER-02) ───
// jimeng-client is @deprecated (replaced by dreamina CLI per lib/jimeng-client.js JSDoc).
// Per locked decision: mark fallback-only at all production call sites with a deduplicated
// module-level warn so operators see it once per process lifetime (no log flood).
let _jimengDeprecateWarned = false;
...
/** Test-only helper: resets the dedup flag so warn-on-first-call can be re-asserted. */
export function _resetJimengDeprecateFlagForTest() { _jimengDeprecateWarned = false; }
...
/**
 * 从 pain-report.json (V6 主路径) / requirement.json (legacy, 有 warn) / pipeline.config.characters (降级兜底, 有 warn) 读取 characters
 */
async function _loadCharactersForGeneration(pipeline) {
  const normalizeChar = (c, i) => ({
    id: c.id || `char-${i + 1}`,
    name: c.name,
  ...
  // Tier 1 (V6 主路径): pain-report.json 的 requirement.characters (pain-discovery 写入)
  try {
    const raw = await readFile(join(pipeline.workdir, 'pain-report.json'), 'utf-8');
    const painReport = JSON.parse(raw);
    if (painReport?.requirement?.characters?.length) {
      return painReport.requirement.characters.map(normalizeChar);
    }
  } catch { /* file missing or unparseable — fallback */ }

  // Tier 2 (legacy fallback, 有 warn): requirement.json (requirement-bible phase 写入)
  try {
    const raw = await readFile(join(pipeline.workdir, 'requirement.json'), 'utf-8');
    const req = JSON.parse(raw);
    if (req?.characters?.length) {
      console.warn(`[character-generation] using legacy requirement.json — migrate to pain-report.json (V6 default). Reason: pain-report.json missing requirement.characters`);
      return req.characters.map(normalizeChar);
    }
  } catch { /* file missing — fallback */ }

  // Tier 3 (degraded 兜底, 有 warn): pipeline.config.characters
  console.warn('[character-generation] pipeline.config.characters fallback in use — no pain-report.json or requirement.json found');
  const chars = pipeline.config?.characters || [];
  return chars.map(normalizeChar);
}


// ─── Quick Task 260702-q6l: PipelineReflector applied-suggestions loader ────
// Non-blocking: any failure (missing files, parse error, reflector throws) is
// silently swallowed. The pipeline run must proceed unaffected.
// Returns the loaded applied suggestions array (may be empty); on any error
// returns []. Also loads prompt-overrides.json into pipeline.appliedReflections
// for prompt-side consumption (TODO: PromptInjector hook is follow-up wiring).
export async function loadAppliedReflections(pipeline) {
  if (!pipeline?.workdir) return [];
  try {
    const { PipelineReflector } = await import('../pipeline-reflector.js');
    const reflector = new PipelineReflector(pipeline.workdir);
    const applied = await reflector.readAppliedSuggestions();
    if (Array.isArray(applied) && applied.length > 0) {
      console.log(`[pipeline] Applied ${applied.length} reflection suggestions (override file at .pipeline-assets/prompt-overrides.json)`);
    }
    // Load prompt-overrides.json if it exists, expose for prompt-side consumption.
    let overrides = null;
    try {
      const raw = await readFile(join(pipeline.workdir, '.pipeline-assets', 'prompt-overrides.json'), 'utf-8');
      overrides = JSON.parse(raw);
    } catch { /* file missing — no overrides */ }
    // Documented field — PromptInjector integration is follow-up wiring.
    pipeline.appliedReflections = { applied, overrides };
    return applied;
  } catch (err) {
    // Silent fail: log at most a warn, never rethrow.
    console.warn(`[pipeline] loadAppliedReflections: silent skip — ${err?.message || err}`);
    return [];
  }
}
...
export const phaseHandlers = {
  ...
  'requirement-bible': {
    after: async (pipeline, phase, phaseConfig) => {
      // Quick Task 260702-q6l: load applied reflection suggestions BEFORE first phase logic.
      // Non-blocking — failure here is swallowed inside loadAppliedReflections.
      try { await loadAppliedReflections(pipeline); } catch { /* silent */ }
    ...
    after: async (pipeline, phase, phaseConfig) => {
      ...
      if (pipeline.config.goldTeam?.baseUrl) {
        try {
          ...
          for (const shot of stsScript.shots || []) {
            const task = await gtClient.submitTask({
              taskType: 'blender_render', priority: 3,
            ...
            previewResults.push({ shot_id: shot.id, taskId: task.taskId });
          }
  ...
  composition: {
    after: async (pipeline, phase, phaseConfig) => {
      ...
      // PIPE-COMPOSE-01: PHASES declares outputFiles ['master.mp4', 'web-preview.mp4'].
      // Composition must write master.mp4 (not final.mp4) so delivery can find it.
      const masterPath = join(pipeline.workdir, 'master.mp4');
      ...
      try {
        ...
        if (composeResult.output) {
          ...
          // PIPE-COMPOSE-01: generate sibling web-preview.mp4 (low-res H.264 transcode).
          // Best-effort: failure is warn-and-continue (T-29-02 mitigation). `-an` drops
          // audio so corrupt/empty audio streams don't break the transcode.
          try {
            ...
            console.warn(`[composition] web-preview 转码降级: ${pvErr.message}`);
          }
        } else {
          // compose() returns {output: null, error} on failure rather than throwing.
          // Treat as degraded — touch placeholders so delivery can still find files.
          console.warn(`[composition] FFmpeg合成未产出 (output=null): ${composeResult.error || 'unknown'}`);
        }
        ...
        console.warn(`[composition] FFmpeg合成降级: ${e.message}`);
      }

      // PIPE-COMPOSE-01: degraded-mode placeholder touch. When compose did not produce
      // a real master.mp4 (threw, or returned output=null), touch 0-byte placeholders so
      // downstream delivery always finds both files. Placeholder touch failure is
      // warn-and-continue — never crash the handler.
      if (!composeSucceeded) {
        for (const placeholder of [masterPath, webPreviewPath]) {
          try { await writeFile(placeholder, ''); }
          catch (touchErr) { console.warn(`[composition] 占位文件 touch 失败 (${placeholder}): ${touchErr.message}`); }
        }
      }
...
返回 JSON: { "score": 0.0-1.0, "details": "评估说明", "issues": [] }`,
  ...
  'consistency-guard': {
    after: async (pipeline, phase, phaseConfig) => {
      ...
      // PIPE-GUARD-01 (Plan 29-03): hermesAudit + collector MUST run before the
      // blocking throw so telemetry captures the failure even when the episode
      // is marked failed. The throw happens after both calls complete.
      ...
      // PIPE-GUARD-01 (Plan 29-03): blocking activation — composition is now a
      // real output phase (Plan 29-01), so consistency-guard no longer defers.
      // Fail writes a marker file (separate from quality-report.json to avoid
      // ordering coupling with composition's later write) and propagates the
      // error to Pipeline.run, which marks the episode failed (lib/pipeline.js
      // runPhase catch block). T-29-08: marker write is wrapped in try/catch —
      // the throw happens AFTER the write attempt regardless, so blocking still
      // propagates even if the marker file is missing.
      if (!stubData.passed) {
  ...
  'delivery': {
    after: async (pipeline, phase, phaseConfig) => {
      ...
      // Step 3: 质量评估 + master.mp4 路径验证 + 完整 quality-report.json
      // Phase 18 实化: 删除 _stub/_pendingRealImplementation,产出完整 quality-report 字段
      // Phase 29-02 PIPE-COMPOSE-02: 对齐 Plan 01 — 验证 master.mp4 (不再验证 final.mp4)
      let qualityReport = {
        overall_score: 0,
        dimensions: {},
        passed: false,
      ...
      // ─── Phase 18 新增 / Phase 29-02 PIPE-COMPOSE-02: master.mp4 路径验证 ───
      // 验证 composition phase 是否产出 master.mp4 (degraded 模式下可能为 0-byte 占位)
      const masterMp4Path = join(pipeline.workdir, 'master.mp4');
      let masterMp4Status = 'absent';
      try {
        const stat = await import('node:fs/promises').then(m => m.stat(masterMp4Path));
        masterMp4Status = 'present';
        qualityReport.master_mp4 = {
          path: masterMp4Path,
        ...
        qualityReport.master_mp4 = {
          path: masterMp4Path,
          status: 'absent',
          note: 'master.mp4 未生成 (degraded 模式或 composition 未执行)',
        };
      }

      // ─── Phase 29-02 PIPE-COMPOSE-02: web-preview.mp4 degrade-tolerant 检查 ───
      // web-preview.mp4 是 best-effort 产物 (degraded 模式可能未生成)。
      // 缺失时只 warn,不 fail —— 显式 non-blocking (CONTEXT D-PIPE-COMPOSE-02)。
      const webPreviewPath = join(pipeline.workdir, 'web-preview.mp4');
      let webPreviewStatus = 'absent';
      try {
        const stat = await import('node:fs/promises').then(m => m.stat(webPreviewPath));
        webPreviewStatus = 'present';
        qualityReport.web_preview_mp4 = {
          path: webPreviewPath,
          size_bytes: stat.size,
          size_mb: Math.round((stat.size / 1024 / 1024) * 100) / 100,
        };
      } catch {
        console.warn('[delivery] web-preview.mp4 缺失 (degrade-tolerant, 不阻断)');
        qualityReport.web_preview_mp4 = {
          path: webPreviewPath,
          status: 'absent',
          note: 'web-preview 未生成 (degrade-tolerant)',
        };
      }

      // 完整 quality-report 字段 (Phase 18: 不再 stub)
      // Phase 29-02 PIPE-COMPOSE-02: 注入 _composition operator-visibility marker
      // (top-level, sibling to _phase, per CONTEXT Claude's Discretion)
      const qualityData = {
        _phase: 'delivery',
        _generatedAt: new Date().toISOString(),
        _composition: {
          delivered_mastermp4: masterMp4Status === 'present',
          delivered_webpreview: webPreviewStatus === 'present',
        },
      ...
      _hermesAudit(hermes, 'delivery', hermesDecisionId, {
        ...
        master_mp4_status: masterMp4Status,
        web_preview_status: webPreviewStatus,
      }, effectiveParams);
