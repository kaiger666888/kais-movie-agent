...
/**
 * 构建空间锚点 prompt（植入道具相对位置）。
 * 将 scene-meta.json 中的 spatial_anchors 转化为 prompt 约束。
 *
 * @param {object} sceneMeta — SceneBible.getSceneMeta() 返回值
 * @param {string} action — 当前动作描述
 * @returns {string} 含空间锚点约束的 prompt 片段
 */
export function buildSpatialAnchorPrompt(sceneMeta, action) {
  const anchors = sceneMeta?.spatial_anchors;
  if (!anchors) return action || '';

  const parts = [];

  // 主角位置
  if (anchors.main_subject_position) {
    parts.push(`subject positioned ${anchors.main_subject_position}`);
  }

  // 道具相对位置
  if (anchors.key_object_positions) {
    parts.push(anchors.key_object_positions);
  }

  // 出入口
  if (anchors.exit_entry_points) {
    parts.push(`entry/exit: ${anchors.exit_entry_points}`);
  }

  // 光源方向（空间锚点中可能包含）
  if (anchors.light_source_direction) {
    parts.push(`light comes from ${anchors.light_source_direction}`);
  }
...
/**
 * 构建光影时间锁 prompt。
 * 将 scene-meta.json 中的 lighting_time_lock 转化为视觉约束。
 *
 * @param {object} sceneMeta — SceneBible.getSceneMeta() 返回值
 * @returns {string} 光影时间约束 prompt 片段（无锚点时返回空串）
 */
export function buildLightingTimeLock(sceneMeta) {
  const lock = sceneMeta?.lighting_time_lock;
  if (!lock) return '';

  const parts = [];

  if (lock.timeOfDay) parts.push(lock.timeOfDay);
  if (lock.colorTemperature) parts.push(lock.colorTemperature);
  if (lock.lightSource) parts.push(lock.lightSource);
  if (lock.shadowDirection) parts.push(lock.shadowDirection);
...
/**
 * 构建动作时序分段 prompt。
 * 将动作按时间段描述，适用于 5-15 秒短视频的时序控制。
 *
 * @param {Array<{start, end, action, cameraMovement?}>} segments — 时序分段
 * @returns {string} 时序动作 prompt（如 "0-5s: character walks to desk. 5-10s: character picks up book. 10-15s: character reads."）
 */
export function buildTemporalSegmentedAction(segments) {
  if (!segments?.length) return '';

  return segments
    .map(seg => {
      const timeRange = `${seg.start}-${seg.end}s`;
      const camera = seg.cameraMovement ? ` (${seg.cameraMovement})` : '';
      return `${timeRange}: ${seg.action}${camera}`;
    })
    .join('. ');
}
...
export default {
  ...
  buildSpatialAnchorPrompt,
  buildLightingTimeLock,
  buildTemporalSegmentedAction,
};
