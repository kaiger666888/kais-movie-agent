/**
 * kais-jimeng — 即梦 API 客户端
 *
 * @deprecated 已被 dreamina CLI 取代。保留此文件仅做兼容参考。
 * 所有新代码应使用 `dreamina` CLI 命令：
 *   - 文生图: dreamina text2image
 *   - 图生图: dreamina image2image
 *   - 视频生成: dreamina multimodal2video / multiframe2video / frames2video
 *   - 超分辨率: dreamina image_upscale
 *
 * 所有 skill 和 pipeline 通过 dreamina CLI 与即梦 API 交互。
 */
...
export class JimengClient {
  ...
  /**
   * Seed 锁定生成 — compositions API，固定 sample_strength 0.3
   * 用于场景 Bible 的种子锁定生成。
   *
   * @param {string} prompt — 生成描述
   * @param {string} seedImageUrl — 种子图 URL
   * @param {object} [options] — { model, ratio, resolution, negative_prompt, seed }
   * @returns {Promise<Array<{url, seed}>>}
   */
  async generateWithSeedLock(prompt, seedImageUrl, options = {}) {
    const {
      model = 'jimeng-5.0',
      ratio = '16:9',
      resolution = '2k',
    ...
    return this.compositions(prompt, {
      images: [seedImageUrl],
      sample_strength: 0.3,
  ...
  /**
   * 批量机位图生成 — 以种子图+氛围图双参考生成多个机位。
   * 用于 SceneBible.generateKeyframes()。
   *
   * @param {string} seedImageUrl — 种子/全景图 URL
   * @param {string} atmosphereUrl — 氛围图 URL
   * @param {Array<{id, prompt, shotType}>} shots — 机位列表
   * @param {object} [options] — { model, ratio, resolution, sample_strength }
   * @returns {Promise<Array<{id, url, seed, prompt, shotType}>>}
   */
  async generateKeyframesBatch(seedImageUrl, atmosphereUrl, shots, options = {}) {
    const {
      model = 'jimeng-5.0',
      ratio = '16:9',
      resolution = '2k',
    ...
    for (const shot of shots) {
      const refImages = [atmosphereUrl, seedImageUrl].filter(Boolean);
      const gen = await this.compositions(shot.prompt, {
        images: refImages,
        sample_strength,
