 ...
 *
 * Phase 13 (2026-06-17): default flipped from 'v8' to 'v2'.
 * V8 still available via explicit `KAI_PIPELINE_MODE=v8` for backward compatibility.
 * See docs/V8-DEPRECATION.md.
 *
 * @param {string} [override]
 * @returns {'v8' | 'v2' | 'parallel'}
 */
export function resolvePipelineMode(override = null) {
  const raw = (override || process.env.KAI_PIPELINE_MODE || 'v2').toLowerCase().trim();
  if (raw === 'v2' || raw === 'v2.0') return 'v2';
  if (raw === 'parallel' || raw === 'ab') return 'parallel';
  if (raw === 'v8' || raw === 'v8.0' || raw === 'legacy') return 'v8';
  // Unknown value → safe default to v2 (Phase 13)
  console.warn(`[v2_pipeline] Unknown KAI_PIPELINE_MODE='${raw}' — defaulting to 'v2'`);
  return 'v2';
}
...
export class V2Pipeline {
  ...
  async _runV2(runInputs) {
    ...
    // Phase 14 fix: thread novelty_constraint + consistency_context into the loop
    await this._execLoopWithCritic(
      'screenplay', 'script_auditor',
      {
        story_kernel: storyKernel,
        novelty_constraint: results.creative_source?.novelty_constraint,
        consistency_context: invariants.getConsistencyContext(),
      },
    ...
    // Layer 4 — audio_pipeline (5 sub-steps: voicer + lip_sync + composer + foley + mixer)
    await this._execNode('audio_pipeline', registry, {
      screenplay_full: results.screenplay?.screenplay_full,
      generated_visuals: results.visual_executor?.generated_visuals,
    }, results, trace, invariants);

    // Layer 5 — editor (with human_review_gate_2 emission)
    await this._execNode('editor', registry, {
      screenplay_full: results.screenplay?.screenplay_full,
      generated_visuals: results.visual_executor?.generated_visuals,
      mixed_audio: results.audio_pipeline,
    }, results, trace, invariants);
    await this._handleHumanGate2(results.editor?.human_review_gate_2, trace);

    // Layer 5 — colorist
    await this._execNode('colorist', registry, {
      edited_sequence: results.editor?.edited_sequence,
    }, results, trace, invariants);

    // Layer 5 — hook_retention (form-specific; skips if not short_drama)
    const formContext = runInputs.form_context || { form: 'short_drama', target_platform: 'douyin' };
    await this._execNode('hook_retention', registry, {
      screenplay_full: results.screenplay?.screenplay_full,
      form_context: formContext,
    }, results, trace, invariants);

    // Layer 6 — quality_gate (final gate)
    await this._execNode('quality_gate', registry, {
      color_graded_sequence: results.colorist?.color_graded_sequence,
      mixed_audio: results.audio_pipeline,
      form_context: formContext,
    }, results, trace, invariants);

    // Layer 6 — compliance_gate (final gate, pre_check + final merged)
    await this._execNode('compliance_gate', registry, {
      quality_approved_sequence: results.quality_gate?.color_graded_sequence || results.colorist?.color_graded_sequence,
      form_context: formContext,
      screenplay_full: results.screenplay?.screenplay_full,
    }, results, trace, invariants);

    // theory_critic — consultative, NOT auto-invoked
    trace.theory_critic_note = 'consultative per META-06; available via V2Pipeline.invokeTheoryCritic(question)';

    trace.completed_at = new Date().toISOString();
    trace.invariants_snapshot = invariants.snapshot();
    trace.layer_4_6_native = true; // Phase 12: full native v2.0 end-to-end
  ...
  /**
   * Handle human_review_gate_2 emission from editor (5-min budget per edges.yaml).
   * Autonomous mode auto-accepts; production defers to creator UI.
   */
  async _handleHumanGate2(gateSpec, trace) {
    if (!gateSpec) return;
    trace.human_gates = trace.human_gates || [];
    trace.human_gates.push({
      gate_type: gateSpec.gate_type,
      review_budget_minutes: gateSpec.review_budget_minutes,
      auto_accepted: gateSpec.auto_accept_in_autonomous_mode,
      timestamp: new Date().toISOString(),
    });
    // Autonomous: auto-accept. Production: yield to UI / await human decision.
  }

  /**
   * Invoke theory_critic consultative API (creator-pulled).
   * Per META-06: NOT auto-invoked by DAG; called by creator UI on demand.
   *
   * @param {string} question
   * @param {object} [pipelineStateSnapshot]
   */
  async invokeTheoryCritic(question, pipelineStateSnapshot = {}) {
    const registry = this.getNodeRegistry();
    const tc = registry.theory_critic;
    if (!tc || typeof tc.consult !== 'function') {
      throw new Error('[v2_pipeline] theory_critic not registered or missing consult()');
    }
    return tc.consult(this, {
      consultation_question: question,
      pipeline_state_snapshot: pipelineStateSnapshot,
    });
  }
  ...
  _buildCriticInputs(generatorId, generatorResult, loopIter, accumulatedCost, invariants) {
    if (generatorId === 'screenplay') {
      // Phase 14 fix: forward consistency_context snapshot so script_auditor's 6th dim evaluates
      return {
        screenplay_full: generatorResult.screenplay_full,
        consistency_context: invariants?.getConsistencyContext?.() || null,
