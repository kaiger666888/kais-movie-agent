 ...
 *
 * 写入路径：HTTP API（/api/canvas/v2/save-v2）— 单一写入路径，
 *   消除 canvasGraph 双写竞态（PIPE-INTEGRITY-01）。
 *   HTTP 不可达时 degrade warn，不抛错（管线继续）。
 * 读取路径：loadGraph 仍走 sqlite3 CLI 直读（read-only，未迁移 per D-PIPE-INTEGRITY-01）。
 *
 * 使用方式：
...
/**
 * Canvas HTTP API base URL — kais-aigc-platform.
 * 与 canvas-sync-hook.js + canvas-client.js DEFAULT_BASE_URL 一致。
 */
const CANVAS_API_BASE_URL =
  process.env.CANVAS_API_BASE_URL || 'http://192.168.71.176:10588';

/**
 * 直接通过 SQLite 读取画布图（read-only，未迁移 per D-PIPE-INTEGRITY-01）。
 * 注意：仅在本机运行时可用。
 */

async function loadGraph(projectId, episodesId) {
  // 使用 sqlite3 CLI（零依赖）— read-only
...
/**
 * 通过 HTTP API 写入画布图（单一写入路径，消除双写竞态）。
 * HTTP 不可达（fetch reject / 5xx / 超时）时 degrade warn，不抛错（D-PIPE-INTEGRITY-01）。
 */
async function saveGraph(projectId, episodesId, graph) {
  ...
  try {
    ...
    await client.saveCanvas(graph);
  } catch (err) {
    // Degrade: 平台不可达/超时/5xx 时不阻断管线（D-PIPE-INTEGRITY-01 + T-28-02/T-28-04）
    console.warn(
      `[canvas-sync] HTTP API unreachable, skipping canvas write: ${err.message}`,
    );
  }
}
...
// Test-only export — exposes private saveGraph for unit testing the HTTP write path.
// Established pattern (see Phase 27 P02 `_resetJimengDeprecateFlagForTest`).
export { saveGraph as __test_saveGraph };
