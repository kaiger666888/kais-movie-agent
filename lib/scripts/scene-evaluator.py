...
def evaluate_single(img_path, description, constraints, api_key):
    """调用视觉模型评估单张图片"""
...
def main():
    if len(sys.argv) < 3:
        print("用法: python3 scene-evaluator.py <spec.json> <img_path_or_dir>")
        print("\nspec.json 示例:")
        print(json.dumps({
            "shots": [
                {"id": "B01", "description": "角色吃面", "constraints": ["手里有筷子", "无多余筷子"]}
            ]
        }, ensure_ascii=False, indent=2))
        sys.exit(1)
    
    spec_path = sys.argv[1]
    target = sys.argv[2]  # 单张图片路径 或 目录
    api_key = get_api_key()
    ...
    for shot in shots:
        ...
        try:
            ...
            eval_text = evaluate_single(img_path, desc, constraints, api_key)
