...
export class QualityGate {
  ...
  constructor(options = {}) {
    ...
    this.visionModel = options.visionModel || process.env.ZHIPU_VISION_MODEL || 'glm-4.6v';
  ...
  async evaluate(inputs, options = {}) {
    ...
    for (const [dimKey, meta] of Object.entries(DIMENSION_META)) {
      try {
        ...
        if (score) {
          ...
          // QUAL-02 (v2.0): API 失败 → score=null(不再用 0.8 兜底伪造)
          // 下游 decide() 与 generateReport() 必须处理 null score 维度
          dimensions[dimKey] = {
            score: null,
            max: meta.max,
            reasons: ['LLM 调用失败 — 评分缺失,不参与门控决策'],
            highlights: [],
            suggestions: [],
            _failed: true,
          };
        ...
        console.warn(`[quality-gate] ${dimKey} 评分异常: ${err.message}`);
        // QUAL-02 (v2.0): 异常 → score=null(不再用 0.8 兜底)
        dimensions[dimKey] = {
          score: null,
          max: meta.max,
          reasons: [`评分异常: ${err.message} — 不参与门控决策`],
          highlights: [],
          suggestions: [],
          _failed: true,
        };
    ...
    // QUAL-02 (v2.0): 全维度失败 → 直接 fail,不再假通过
    const failedCount = Object.values(dimensions).filter(d => d._failed).length;
    if (failedCount === Object.keys(dimensions).length) {
      const allFailedErr = new Error('质量门控失败:所有维度 LLM 评分均失败,无法生成可信分数');
      allFailedErr.code = 'QUALITY_GATE_ALL_DIMENSIONS_FAILED';
      throw allFailedErr;
    }
    if (failedCount > 0) {
      console.warn(`[quality-gate] ⚠ ${failedCount}/${Object.keys(dimensions).length} 维度评分失败 — 总分仅基于已成功维度`);
    }

    // 计算加权总分（蓝图权重或默认满分制）
    // QUAL-02 (v2.0): 跳过 score=null 的维度,仅基于已成功维度算分
    let totalScore;
    const scoredDims = Object.values(dimensions).filter(d => d.score !== null && d.score !== undefined);
    if (blueprintWeights && Object.keys(blueprintWeights).length > 0) {
      // 使用蓝图权重加权 — 按已成功维度的权重归一化
      totalScore = 0;
      let totalWeight = 0;
  ...
  decide(result) {
    ...
    // 一票否决：任一已评分维度 < critical(QUAL-02: null score 维度跳过)
    for (const [dimKey, dim] of Object.entries(dimensions)) {
      if (dim.score === null || dim.score === undefined) continue;  // QUAL-02: 失败维度不参与否决
  ...
  generateReport(result) {
    ...
    for (const [dimKey, dim] of Object.entries(dimensions)) {
      ...
      if (dim.score === null || dim.score === undefined) {
        report += `  ${meta.emoji} ${meta.label.padEnd(8, '　')}: --/${dim.max}  ⚠️ 评分失败(不参与门控)\n`;
        continue;
      }
      const percentage = (dim.score / dim.max) * 100;
      let status = '✅';
