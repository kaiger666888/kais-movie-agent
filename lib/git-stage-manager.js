...
const STAGE_REGISTRY = {
  // === V6 20-step pipeline stages ===
  'pain-discovery':      { label: '痛点调查',           glob: ['pain-report.json'] },
  'topic-selection':     { label: '主题选择',           glob: ['selected-topic.json'] },
  'outline':             { label: '大纲生成',           glob: ['outline-candidates.json'] },
  'outline-selection':   { label: '大纲选择',           glob: ['selected-outline.json'] },
  'script':              { label: '剧本生成',           glob: ['script-candidates.json'] },
  'script-selection':    { label: '剧本选择',           glob: ['selected-script.json'] },
  'character-gen':       { label: '主角生成',           glob: ['character-candidates.json', 'assets/characters/'] },
  'character-select':    { label: '主角选择',           glob: ['soul-pack.json'] },
  'scene-gen':           { label: '场景生成',           glob: ['scene-candidates.json', 'assets/scenes/'] },
  'scene-select':        { label: '场景选择',           glob: ['geometry-bed.json'] },
  'spatio-temporal':     { label: '时空剧本',           glob: ['sts-script.json'] },
  'script-lock':         { label: '剧本锁定审核',       glob: ['script-locked.json'] },
  'seed-skeleton':       { label: '种子骨架',           glob: ['seed-skeleton-pack.json', 'shot_seed_frames/', 'temp_dialogue/', 'bgm_segments/', 'ambience_base/'] },
  'motion-preview':      { label: '运镜定稿与动态预览', glob: ['camera-plan.json', 'motion-preview.mp4', 'rough-mix.mp3'] },
  'ai-preview':          { label: 'AI风格化预览',       glob: ['preview-pack/', 'seedance-input-pack.json', 'audio_plan.json'] },
  'consistency-guard':   { label: '一致性守护检查',     glob: ['consistency-pass.json'] },
  'cloud-production':    { label: '云端终版视频',       glob: ['final-shots/video/'] },
  'final-audio':         { label: '本地BGM与声音闭环',   glob: ['final-shots/audio-stems/'] },
  'composition':         { label: '剪辑合成',           glob: ['master.mp4', 'web-preview.mp4'] },
  'delivery':            { label: '质检与交付',         glob: ['quality-report.json'] },
};
