...
// ─── Artifact 提取 & 3 层节点构建 (纯函数, 供测试) ────────

/**
 * extractArtifacts — 从 phase result 中抽取所有产出物为扁平数组。
 *
 * 查找顺序:
 *   1. result.summary.result.outputs  (kais-movie-agent 深层路径)
 *   2. result.summary                 (扁平 phase 数据 — 顶层 key 直接是数组)
 *
 * 对每个值为数组的 key,把每个元素抽取为 artifact:
 *   { output_key, label, thumbnailUrl, filePath, tags, name, filename }
 *
 * @param {object} result - pipeline 传给 onPhaseComplete 的 { summary, metrics, review }
 * @returns {Array<object>} 扁平 artifact 列表 (可能为空)
 */
export function extractArtifacts(result) {
  if (!result || typeof result !== 'object') return [];

  // 1) 深层路径: result.summary.result.outputs
  let outputs = null;
  try {
    const deep = result?.summary?.result?.outputs;
    if (deep && typeof deep === 'object' && !Array.isArray(deep)) outputs = deep;
  } catch { /* ignore */ }

  // 2) 回退: result.summary 顶层 (若存在 array 值)
  if (!outputs) {
    const summary = result?.summary;
    if (summary && typeof summary === 'object' && !Array.isArray(summary)) {
      const hasArrayValue = Object.values(summary).some(v => Array.isArray(v));
      if (hasArrayValue) outputs = summary;
    }
  }
  ...
  const isImageExt = (p) =>
    typeof p === 'string' && /\.(png|jpe?g|webp|gif|bmp|svg)$/i.test(p);

  // Metadata keys to skip when recursing into nested objects
  const SKIP_KEYS = new Set([
    'type', 'version', 'expert', 'episode', 'created_at', 'film_title', 'film_brand',
    'style_core_global', 'model', 'resolution', 'consistency_note', 'batch_strategy',
    'golden_standard_rule', 'meta', 'metadata', 'cross_character_clip_i_target',
    'family_resemblance_rules', 'distinct_identity_rules',
  ]);

  const artifacts = [];

  function pushItem(output_key, item, index) {
    if (item == null) return;
    if (typeof item !== 'object' || Array.isArray(item)) {
      // primitive or array — stringify
      artifacts.push({
        output_key,
        label: String(item),
    ...
    const filePath = item.filePath || item.file || item.path;
    const thumbnailUrl =
      item.thumbnailUrl || item.imageUrl || item.image ||
      (isImageExt(filePath) ? filePath : undefined);
    const label =
      item.name || item.label || item.title || item.id || `${output_key}[${index}]`;
    const name = item.name || item.label;
    const filename =
      typeof filePath === 'string' ? filePath.split('/').pop().split('\\').pop() : undefined;
    artifacts.push({
      output_key,
      label,
      thumbnailUrl,
  ...
  /**
   * walk — recursively extract arrays from nested dicts.
   * Uses LEAF key name as output_key (matching 9999 pattern where
   * character_bible.characters → output_key="characters").
   */
  function walk(obj, depth) {
    ...
    for (const [key, val] of Object.entries(obj)) {
      ...
      if (Array.isArray(val)) {
        val.forEach((item, i) => pushItem(key, item, i));
...
/**
 * computePhaseIndex — Phase 在画布上的横向列号 (X 坐标基准)。
 * 优先用 stageOrder;否则从 phase.id (如 "p01"/"p04") 抽取数字。
 * @param {object} phase
 * @returns {number}
 */
export function computePhaseIndex(phase) {
  if (phase?.stageOrder && phase.stageOrder > 0) return phase.stageOrder;
  const m = String(phase?.id || '').match(/p?(\d+)/i);
  if (m) return parseInt(m[1], 10);
  return 0;
}

/**
 * buildPhaseGraph — 为单个 phase 构建 3 层 (Zone → Summary → Artifacts)
 * 节点 + 连线结构。纯函数,不触网络,便于测试。
 *
 * 节点 ID 规范 (匹配 project 9999 pattern):
 *   - Zone:      phase.id                       (如 "p01" / "character-generation")
 *   - Summary:   "sum-" + phase.id
 *   - Artifact:  "a-" + phase.id + "-art" + idx
 *
 * 连线 ID 规范:
 *   - zone→summary:  "zl2-{phase.id}-sum-{phase.id}"          dataType="output"
 *   - zone→artifact: "zc-{phase.id}-a-{phase.id}-art{idx}"   dataType="output"
 *   - prev→zone:     "fl-{prevPhase.id}-{phase.id}"           dataType="flow"
 *
...
export function buildPhaseGraph({
  ...
  const zoneNode = {
    ...
    state,
    branchId: 'main',
    data: { label: mapped.label, phase: phaseGroup, state },
  };

  const summaryNode = {
    id: `sum-${phase.id}`,
    type: nodeType,
    position: { x: baseX, y: 80 },
  ...
  const artifactNodes = artifacts.map((art, index) => ({
    id: `a-${phase.id}-art${index}`,
    type: nodeType,
    position: {
      x: baseX + (index % 4) * 280,
  ...
  const links = [
    ...
    ...artifacts.map((_, index) => ({
      id: `zc-${phase.id}-a-${phase.id}-art${index}`,
      source: phase.id,
      target: `a-${phase.id}-art${index}`,
      branchId: 'main',
      dataType: 'output',
    })),
  ];
  ...
  return { nodes: [zoneNode, summaryNode, ...artifactNodes], links };
}
 ...
 * @param {string} options.baseUrl - 画布服务地址 (e.g. 'http://localhost:10588')
...
export function createCanvasSync({
  ...
  // ─── 核心: batchUpsert (单次 load+save 原子写入) ─────────

  /**
   * batchUpsert — 一次性把多个节点 + 连线写入画布。
   * 单次 loadGraph + 单次 saveGraph,避免 N 节点 → N×HTTP 请求。
   * - 节点: 按 id upsert (存在则合并 data, 不存在则 push)
   * - 连线: 按 id 去重 push
   */
  async function batchUpsert(nodes = [], links = []) {
    const graph = await loadGraph() || { nodes: [], links: [], branches: [], variantGroups: [] };
    for (const node of nodes) {
      ...
      if (idx >= 0) {
        graph.nodes[idx] = {
          ...graph.nodes[idx],
          ...node,
          data: { ...graph.nodes[idx].data, ...node.data },
          position: node.position || graph.nodes[idx].position,
        };
      } else {
   ...
   *
   * 新行为 (V6 多产出物同步): 每个 phase 在画布上展开为 3 层结构,
   * 匹配 project 9999 pattern:
   *   1. Zone node    — phase 头部 (横向列, X = phaseIndex * 1300)
   *   2. Summary node — phase 级摘要 (assetId, "N artifacts" 描述)
   *   3. Artifact nodes — 每个产出物一个节点 (从 result.summary 抽取)
   *
   * 用 batchUpsert 单次 load+save 原子写入,避免 N 节点 × N HTTP。
   *
  ...
  async function onPhaseComplete(phase, result) {
    ...
    const phaseIndex = computePhaseIndex(phase);
    const phaseState =
      result?.review?.action === 'awaiting_review' ? 'pending' : 'success';

    // 抽取产出物 (V6: 一个 phase 可能产出多个 artifact)
    const artifacts = extractArtifacts(result);

    // 找上一个 phase (用 stageOrder 排序,与 reportPhaseStart 行为一致)
    const prevPhaseId = nodeMap.get(phase.id)?.prevPhaseId || (() => {
      const known = [...nodeMap.entries()]
        .filter(([id]) => id !== phase.id)
        .sort((a, b) => (b[1]?.stageOrder ?? -1) - (a[1]?.stageOrder ?? -1));
      return known.length > 0 ? known[0][0] : null;
    })();

    // ★ 注册资产到 o_assets (拿到 assetId) — 保留以兼容资产库
    const asset = await registerPhaseAsset(phase, mapped, result);

    // ★ 构建 3 层节点 + 连线 (纯函数, 便于测试)
    ...
    // ★ 单次原子批写
    await batchUpsert(nodes, links);

    // 记录节点信息 (供 onProgress / 下一个 phase 找 prev 用)
    nodeMap.set(phase.id, {
      nodeId: phase.id, // 新: zone 节点 ID = phase.id (而非 n-{phase.id})
      position: { x: phaseIndex * 1300, y: 0 },
      prevPhaseId,
      stageOrder: phaseIndex,
