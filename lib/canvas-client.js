 ...
 */
const DEFAULT_BASE_URL = 'http://192.168.71.176:10588';
 ...
 *     baseUrl: 'http://192.168.71.166:3000',
 *     projectId: 123,
 *     episodesId: 456,
 *     pipelineId: 'pipe_xxx',
 *   });
...
export class CanvasClient {
   ...
   * @param {string} [options.baseUrl='http://192.168.71.166:3000'] - 画布服务地址
   ...
   * @param {string} [options.apiPrefix='/api/v2/canvas'] - REST API 前缀（v2 设计）
   * @param {string} [options.legacyApiPrefix='/api/canvas'] - 兼容 v1 路径（load/save/review）
   */
  constructor({
  ...
  async saveCanvas(graph) {
    ...
    return this._request(`${this._apiPrefix}/save`, {
      projectId: this._projectId,
      episodesId: this._episodesId,
      graph,
    });
  ...
  /**
   * 创建单个节点
   * 优先调用 POST /api/v2/canvas/nodes，若 404 则降级为 patchCanvas
   * @param {object} node - { id, type, position, size, data, state, groupId?, reviewStatus? }
   */
  async addNode(node) {
    this._requireContext();
  ...
  /**
   * 批量创建节点
   * 优先 PATCH /api/v2/canvas/nodes/batch，404 时降级为 patchCanvas
  ...
  /**
   * 更新节点状态
   * PATCH /api/v2/canvas/nodes/:nodeId
   * @param {string} nodeId
  ...
  async updateNodeState(nodeId, state, progress) {
    this._requireContext();
    if (!nodeId) throw new CanvasClientError('updateNodeState: nodeId 必填');
    ...
    try {
      ...
      if (err.status === 404 || err.status === 405) {
        // 降级：通过 patchCanvas 更新单个节点的 state 字段
        return this.patchCanvas({
          nodes: [{ id: nodeId, state, progress }],
        });
      }
      throw err;
    }
  ...
  /**
   * 创建连线
   * POST /api/v2/canvas/links
   * @param {object} link - { id, source, target, dataType, sourceHandle?, targetHandle? }
   */
  async addLink(link) {
    this._requireContext();
    ...
    try {
      ...
      if (err.status === 404 || err.status === 405) {
        return this.patchCanvas({ edges: [link], links: [link] });
      }
      throw err;
    }
  }
  ...
  /**
   * 创建分支
   * POST /api/v2/canvas/branches
   * @param {object} branch - { id, name, parentNodeId, status, metadata? }
   */
  async createBranch(branch) {
    this._requireContext();
    if (!branch?.id) throw new CanvasClientError('createBranch: branch.id 必填');
  ...
  async selectVariantWinner(groupId, winnerNodeId) {
    ...
    try {
      return await this._request(
        `${this._apiPrefix}/variant-groups/${encodeURIComponent(groupId)}/winner`,
        {
          projectId: this._projectId,
          episodesId: this._episodesId,
          winnerNodeId,
  ...
  /**
   * 请求画布自动布局
   * POST /api/v2/canvas/layout
   * @param {object} [hints] - 布局提示（如 layoutType, direction）
   * @returns {Promise<object>} 布局后的 graph 或 layout 元数据
   */
